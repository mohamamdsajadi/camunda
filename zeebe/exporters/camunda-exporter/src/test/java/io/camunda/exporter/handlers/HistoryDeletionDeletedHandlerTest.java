...
class HistoryDeletionDeletedHandlerTest {
  ...
  private final String indexName = "test-delete-history";
        ...
        generateRecord(
            b ->
                b.withValue(recordValue)
                    .withKey(Long.parseLong(EVENT_KEY))
                    .withBatchOperationReference(BATCH_OPERATION_KEY));
    final var historyDeletionEntity = new HistoryDeletionEntity().setId(EVENT_KEY);

    // when
    underTest.updateEntity(record, historyDeletionEntity);

    // then
    assertThat(historyDeletionEntity.getId())
        .isEqualTo(String.format(BATCH_OPERATION_KEY + "_" + RESOURCE_KEY));
  ...
  void shouldCreateNewEntity() {
    // when
    final var result = underTest.createNewEntity(EVENT_KEY);

    // then
    assertThat(result).isNotNull();
    assertThat(result.getId()).isEqualTo(EVENT_KEY);
  }
